...
- A typedef's name must start by $\mathrm{t}_{-}$.
...
A carryall or bad function usually comes with names that are not explicit such as $f 1, f 2 \ldots$ for the function and $a, b, c, \ldots$ for the variables names. A function whose only goal is to avoid the norm, without a unique logical purpose, is also considered as a bad function. Please remind that it is desirable to have clear and readable functions that achieve a clear and simple task each. Avoid any code obfuscation techniques, such as the one-liner, ... .
...
- In the case of a multibinary project, in addition to the above rules, you must have a rule for each binary (eg: $\$($ NAME_1) $, \$($ NAME_2 $), \ldots)$. The "all" rule will compile all the binaries, using each binary rule.
